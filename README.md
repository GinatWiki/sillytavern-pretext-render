...
5. 之后拖面板内上沿的 ⠿ 手柄即可移动，拖面板右下角可调整宽高；位置和尺寸自动记忆，重启不丢失；浏览器窗口缩放时会自动把悬浮面板限制在可视区内，拖拽时也不会把整个面板拖出屏幕；窗口重新放大后会按保存位置恢复，靠近右/下边缘的面板会继续保持吸附
6. 其他扩展添加的面板同样可拾取；已注册的面板显示在设置列表中，可点 ✕ 移除（连同位置记录）

手柄是与面板同宽的整条横栏（类似浏览器标签栏），嵌在面板内顶部或底部，按钮均匀分布；栏位配色跟随 ST 主题（SmartTheme 变量 + 模糊，与顶栏一致）；面板会自动多出一条 16px 边距容纳手柄，不遮挡内容，也不挡右下角调整区。按钮：
